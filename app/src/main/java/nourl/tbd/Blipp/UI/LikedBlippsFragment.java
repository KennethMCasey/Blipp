...
public class LikedBlippsFragment extends Fragment implements BlipGetterCompletion {
    ...
    private void getBlips(String blipToStartAt)
    {
        ...
        if (order.getSelectedItemPosition() == 0) new BlipGetter(BlipGetter.Section.LIKED_BLIPS, BlipGetter.Order.MOST_RECENT, null, this, blipToStartAt, 20).execute();

        else if (order.getSelectedItemPosition() == 1) new BlipGetter(BlipGetter.Section.LIKED_BLIPS, BlipGetter.Order.MOST_LIKED, null, this, blipToStartAt, 20).execute();
    }
